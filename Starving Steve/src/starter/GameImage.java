...
public class GameImage extends GImage {
	...
	public void changeAlpha(int a) {
		expArray = origImage.getPixelArray();
		for (int i =0; i < expArray.length; i++ ) {
			for (int j =0; j < expArray[0].length;j++) {
				expArray[i][j] = GImage.createRGBPixel(GImage.getRed(expArray[i][j]),GImage.getGreen(expArray[i][j]),GImage.getBlue(expArray[i][j]),a);
			}
		}
		setnewImage(new GImage(expArray));
		BufferedImage bImage = toBufferedImage(origImage.getImage());
		setImage(bImage);
		initGameImage();
		
//		System.out.println(origImage.getWidth() + " " + origImage.getHeight());
//		this.origImage.setSize(this.origImage.getWidth()+18, this.origImage.getHeight()+18);
	}
	...
	public void setnewImage(GImage g) {
		this.origImage = g;
	}
	...
	private GPolygon createBoundedPolygon(GImage rect) {
		GPolygon p = new GPolygon(rect.getX()+rect.getWidth()/2, rect.getY()+rect.getHeight()/2);
	...
	public GameImage(String arg0, boolean arg1) {
		super(arg0);
		origImage = new GImage(arg0);
		exp = arg1;
		initGameImage();
...
//		double temp = 0.5;
//		bGr.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, (float) temp));
	...
	public GPolygon getPolygonBounds() {
		return poly;
	}
}
